...
export function createApp(s:Service){
 ...
 app.patch('/v1/devices/:sn/channels/:channel_id',async(req,res)=>{const channelId=z.coerce.number().int().positive().parse(req.params.channel_id);const b=z.object({alias:z.string().trim().max(100)}).strict().parse(req.body);ok(res,await s.updateChannelAlias(sn(req),channelId,b.alias,req.user!));});
