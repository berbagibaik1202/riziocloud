...
export function createApp(s:Service){
 ...
 app.get('/v1/admin/users',async(_req,res)=>ok(res,await s.db.query('SELECT id,email,name,role,status,created_at FROM users ORDER BY created_at DESC LIMIT 1000')));
 ...
 app.patch('/v1/admin/devices/:sn',async(req,res)=>{const b=z.object({disabled:z.boolean()}).strict().parse(req.body);const d=await s.getDevice(sn(req));await s.db.transaction(async tx=>{await tx.execute('UPDATE devices SET disabled=?,online=IF(?,FALSE,online) WHERE id=?',[b.disabled,b.disabled,d.id]);if(b.disabled)await tx.execute("UPDATE device_commands SET status='failed',error='DEVICE_DISABLED' WHERE device_id=? AND status IN ('pending','sent')",[d.id]);});await s.audit('device.disabled',d.id,req.user!.id,b);ok(res,publicDevice({...d,...b}));});
