...
export const channelSchema=z.object({id:z.number().int().positive(),pin:z.number().int().min(0).max(39),name:z.string().min(1).max(64),type:z.enum(['switch','pwm','sensor']),active_low:z.boolean()}).strict();
export const inventorySchema=z.object({sn:z.string().regex(/^[A-Z0-9-]{3,64}$/),name:z.string().min(1).max(100),model:z.string().min(1).max(64),hardware_version:z.string().min(1).max(32),firmware_version:z.string().min(1).max(32).default('unknown'),capabilities:z.record(z.unknown()).default({}),channels:z.array(channelSchema).max(40).refine(v=>new Set(v.map(c=>c.pin)).size===v.length&&new Set(v.map(c=>c.id)).size===v.length,'Duplicate channel pin/id'),device_key:z.string().regex(/^[a-f0-9]{64}$/).optional(),claim_code:z.string().min(16).max(128).optional(),setup_code:z.string().min(16).max(63).optional()}).strict();
...
export const commandSchema=z.object({command:z.enum(['gpio.set','system.reboot','system.factory_reset','firmware.update']),pin:z.number().int().min(0).max(39).optional(),state:z.boolean().optional(),firmware_id:z.string().uuid().optional(),request_id:z.string().uuid().optional()}).strict();
export type CommandInput=z.infer<typeof commandSchema>;
export function publicUser(u:User){return {id:u.id,email:u.email,name:u.name,role:u.role,status:u.status};}
export function publicDevice(d:any){return {sn:d.sn,name:d.name,model:d.model,hardware_version:d.hardware_version,firmware_version:d.firmware_version,online:!!d.online&&!d.disabled,disabled:!!d.disabled,last_seen:d.last_seen,capabilities:json(d.capabilities),channels:json(d.channels),state:json(d.state??{})};}
export class Service {
 ...
 async claim(sn:string,code:string,u:User){return this.db.transaction(async tx=>{
  const [d]=await tx.query('SELECT * FROM devices WHERE sn=? FOR UPDATE',[sn]);must(d,404,'DEVICE_NOT_FOUND');must(!d.disabled,403,'DEVICE_DISABLED');must(!d.owner_user_id,409,'DEVICE_ALREADY_CLAIMED');must(d.claim_code_hash&&equal(d.claim_code_hash,sha256(code)),400,'INVALID_CLAIM_CODE');
  const r=await tx.execute('UPDATE devices SET owner_user_id=?,claim_code_hash=NULL WHERE id=? AND owner_user_id IS NULL AND claim_code_hash=?',[u.id,d.id,d.claim_code_hash]);must(r.affectedRows===1,409,'DEVICE_ALREADY_CLAIMED');await tx.execute('INSERT INTO device_logs(device_id,user_id,event_type,payload) VALUES (?,?,\'claim\',\'{}\')',[d.id,u.id]);return publicDevice({...d,owner_user_id:u.id});
 });}
 async unclaim(sn:string,password:string,u:User){const [account]=await this.db.query<User>('SELECT * FROM users WHERE id=?',[u.id]);must(account?.password_hash&&await bcrypt.compare(password,account.password_hash),401,'AUTH_INVALID');return this.db.transaction(async tx=>{const [d]=await tx.query('SELECT * FROM devices WHERE sn=? FOR UPDATE',[sn]);must(d,404,'DEVICE_NOT_FOUND');must(d.owner_user_id===u.id,403,'DEVICE_NOT_OWNED');const code=secret();await tx.execute('UPDATE devices SET owner_user_id=NULL,claim_code_hash=? WHERE id=?',[sha256(code),d.id]);await tx.execute("UPDATE device_commands SET status='failed',error='DEVICE_UNCLAIMED' WHERE device_id=? AND status IN ('pending','sent')",[d.id]);await tx.execute('INSERT INTO device_logs(device_id,user_id,event_type,payload) VALUES (?,?,\'unclaim\',\'{}\')',[d.id,u.id]);return {sn,claim_code:code};});}
 async createDevice(body:unknown,u:User){const b=inventorySchema.parse(body);must(b.sn!==this.config.MQTT_USERNAME,400,'RESERVED_DEVICE_SN');const key=b.device_key??secret(),code=b.claim_code??secret(),setup=b.setup_code??secret().slice(0,48);must(key!==code&&key!==setup&&code!==setup,400,'CREDENTIALS_MUST_DIFFER');const id=randomUUID();await this.db.execute('INSERT INTO devices(id,sn,device_key_encrypted,claim_code_hash,name,model,hardware_version,firmware_version,capabilities,channels) VALUES (?,?,?,?,?,?,?,?,?,?)',[id,b.sn,encrypt(key,this.config.CREDENTIAL_ENCRYPTION_KEY),sha256(code),b.name,b.model,b.hardware_version,b.firmware_version,JSON.stringify(b.capabilities),JSON.stringify(b.channels)]);await this.audit('inventory.created',id,u.id,{sn:b.sn});return {device:publicDevice({...b,id,online:false,disabled:false,last_seen:null}),production_credentials:{sn:b.sn,device_key:key,claim_code:code,setup_code:setup}};}
 async rotateClaimCode(sn:string,u:User){const code=secret();await this.db.transaction(async tx=>{const [d]=await tx.query('SELECT id,owner_user_id,disabled FROM devices WHERE sn=? FOR UPDATE',[sn]);must(d,404,'DEVICE_NOT_FOUND');must(!d.owner_user_id&&!d.disabled,409,'DEVICE_ALREADY_CLAIMED');await tx.execute('UPDATE devices SET claim_code_hash=? WHERE id=?',[sha256(code),d.id]);await tx.execute('INSERT INTO device_logs(device_id,user_id,event_type,payload) VALUES (?,?,?,?)',[d.id,u.id,'claim_code.rotated','{}']);});return {sn,claim_code:code};}
 ...
 async command(sn:string,input:unknown,u:User,admin=false){const b=commandSchema.parse(input);const requestId=b.request_id??randomUUID();
  const result=await this.db.transaction(async tx=>{
   ...
   if(b.command==='gpio.set'){must(b.pin!==undefined&&b.state!==undefined&&!b.firmware_id,400,'INVALID_COMMAND');must(json<any[]>(d.channels).some(c=>c.pin===b.pin&&c.type==='switch'),400,'INVALID_GPIO');payload.pin=b.pin;payload.state=b.state;}
