...
export const inventorySchema=z.object({sn:z.string().regex(/^[A-Z0-9-]{3,64}$/),name:z.string().min(1).max(100),model:z.string().min(1).max(64),device_type:z.enum(['relay','switch','sensor','other']).default('relay'),relay_type:z.enum(relayTypes).nullable().optional(),dht11_pin:z.number().int().min(0).max(39).optional(),hardware_version:z.string().min(1).max(32),firmware_version:z.string().min(1).max(32).default('unknown'),capabilities:z.record(z.unknown()).default({}),channels:z.array(channelSchema).max(40).refine(v=>new Set(v.map(c=>c.pin)).size===v.length&&new Set(v.map(c=>c.id)).size===v.length,'Duplicate channel pin/id'),device_key:z.string().regex(/^[a-f0-9]{64}$/).optional(),setup_code:z.string().min(16).max(63).optional()}).strict().superRefine((v,ctx)=>{if(v.device_type==='relay'||v.device_type==='switch'){const relayType=v.relay_type;if(!relayType)ctx.addIssue({code:'custom',path:['relay_type'],message:'relay_type is required'});else{const n=Number(relayType.split('_')[1]?.replace('ch','')??0);if(v.channels.length!==n)ctx.addIssue({code:'custom',path:['channels'],message:`${relayType} requires ${n} channels`});}}if(v.device_type==='sensor'){if(v.dht11_pin===undefined)ctx.addIssue({code:'custom',path:['dht11_pin'],message:'dht11_pin is required for sensor devices'});if(v.relay_type!==undefined&&v.relay_type!==null)ctx.addIssue({code:'custom',path:['relay_type'],message:'relay_type is not valid for sensor devices'});if(v.channels.some(c=>c.type!=='sensor'))ctx.addIssue({code:'custom',path:['channels'],message:'Sensor devices require sensor channels'});}});
...
export function publicDevice(d:any){return {sn:d.sn,name:d.name,model:d.model,device_type:d.device_type??'relay',relay_type:d.relay_type??null,dht11_pin:d.dht11_pin??null,hardware_version:d.hardware_version,firmware_version:d.firmware_version,online:!!d.online&&!d.disabled,disabled:!!d.disabled,last_seen:d.last_seen,capabilities:json(d.capabilities),channels:json(d.channels),state:json(d.state??{})};}
export class Service {
 ...
 async createDevice(body:unknown,u:User){const b=inventorySchema.parse(body);must(b.sn!==this.config.MQTT_USERNAME,400,'RESERVED_DEVICE_SN');const key=b.device_key??secret(),setup=b.setup_code??secret().slice(0,48);must(key!==setup,400,'CREDENTIALS_MUST_DIFFER');const id=randomUUID();await this.db.execute('INSERT INTO devices(id,sn,device_key_encrypted,claim_code_hash,name,device_type,relay_type,dht11_pin,model,hardware_version,firmware_version,capabilities,channels) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)',[id,b.sn,encrypt(key,this.config.CREDENTIAL_ENCRYPTION_KEY),null,b.name,b.device_type,b.relay_type??null,b.dht11_pin??null,b.model,b.hardware_version,b.firmware_version,JSON.stringify(b.capabilities),JSON.stringify(b.channels)]);await this.audit('inventory.created',id,u.id,{sn:b.sn,device_type:b.device_type,relay_type:b.relay_type,dht11_pin:b.dht11_pin});return {device:publicDevice({...b,id,online:false,disabled:false}),production_credentials:{sn:b.sn,device_key:key,setup_code:setup}};}
