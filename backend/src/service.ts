...
export const inventorySchema=z.object({sn:z.string().regex(/^[A-Z0-9-]{3,64}$/),name:z.string().min(1).max(100),model:z.string().min(1).max(64),hardware_version:z.string().min(1).max(32),firmware_version:z.string().min(1).max(32).default('unknown'),capabilities:z.record(z.unknown()).default({}),channels:z.array(channelSchema).max(40).refine(v=>new Set(v.map(c=>c.pin)).size===v.length&&new Set(v.map(c=>c.id)).size===v.length,'Duplicate channel pin/id'),device_key:z.string().regex(/^[a-f0-9]{64}$/).optional(),setup_code:z.string().min(16).max(63).optional()}).strict();
...
export class Service {
 ...
 async createDevice(body:unknown,u:User){const b=inventorySchema.parse(body);must(b.sn!==this.config.MQTT_USERNAME,400,'RESERVED_DEVICE_SN');const key=b.device_key??secret(),setup=b.setup_code??secret().slice(0,48);must(key!==setup,400,'CREDENTIALS_MUST_DIFFER');const id=randomUUID();await this.db.execute('INSERT INTO devices(id,sn,device_key_encrypted,claim_code_hash,name,model,hardware_version,firmware_version,capabilities,channels) VALUES (?,?,?,?,?,?,?,?,?,?)',[id,b.sn,encrypt(key,this.config.CREDENTIAL_ENCRYPTION_KEY),null,b.name,b.model,b.hardware_version,b.firmware_version,JSON.stringify(b.capabilities),JSON.stringify(b.channels)]);await this.audit('inventory.created',id,u.id,{sn:b.sn});return {device:publicDevice({...b,id,online:false,disabled:false}),production_credentials:{sn:b.sn,device_key:key,setup_code:setup}};}
