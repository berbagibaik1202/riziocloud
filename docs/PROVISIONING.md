...
4. Konfigurasi channels, reset_pin, dan `dht11_pin` sesuai PCB. Contoh memakai GPIO14 untuk DHT11; hubungkan VCC 3.3V, GND, dan DATA ke pin tersebut (gunakan resistor pull-up sekitar 4.7K–10K bila modul DHT11 tidak memilikinya). Pin DHT11 tidak boleh sama dengan relay atau reset. Default tombol GPIO0 ke GND dengan pull-up internal; tekan hanya setelah boot (GPIO0 LOW saat power-on dapat memasuki bootloader). ESP8266 dan ESP32 memakai nomor GPIO, bukan tulisan D1 pada board.
